...
function updateMapLayers(selectedLayers) {
  ...
  const removeLayersSet = new Set(
    [...gVisibleLayers].filter((x) => !newSelection.has(x))
  );
    ...
    .done(function (geo) {
      if (r.type === "Polygon" || r.type === "MultiPolygon") {
        gCollection[r.shapefile] = L.geoJson(geo, {
          ...
          .bindPopup(`${r.origname}<br>Group: ${r.pid}`, {
            sticky: true,
            opacity: 0.5,
          });
      } else {
...
const crosshair = new L.marker(map.getCenter(), {
  icon: crosshairIcon,
  interactive: false,
});
